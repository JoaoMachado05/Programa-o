...
import "./VisualizarAvarias.css";

const VisualizarAvarias = ({ isOpen, onClose, trafficLightId }) => {
  ...
  useEffect(() => {
    if (isOpen && trafficLightId) {
      fetchAvarias();
    }
  }, [isOpen, trafficLightId, activeTab]);
  ...
  const fetchAvarias = async () => {
    ...
    try {
      ...
      if (activeTab === "ativas") {
        endpoint = `${API_BASE_URL}/traffic-lights/${trafficLightId}/avarias/ativas`;
      } else if (activeTab === "resolvidas") {
        endpoint = `${API_BASE_URL}/traffic-lights/${trafficLightId}/avarias/resolvidas`;
      } else {
        // Se por algum motivo a tab não for reconhecida, buscar todas
        endpoint = `${API_BASE_URL}/traffic-lights/${trafficLightId}/avarias`;
      }
  ...
  const resolverAvaria = async (avariaId) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/traffic-lights/avarias/${avariaId}/resolver`, {
        method: 'PUT',
        headers: {
...
export default VisualizarAvarias;